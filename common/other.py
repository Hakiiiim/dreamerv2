...
def static_scan(fn, inputs, start, reverse=False):
    last = start
    # outputs take the same size as start's entries: stoch, deter, mean, std
    ...
    for index in indices:
        ...
        # apply the function fn (img_step "prior") on last (state) and inp (action)
        # -> prior = {'stoch': stoch, 'deter': deter, **stats}
        last = fn(last, inp)

        # flatten returns [stoch, deter, mean, std], append outputs sub-elements
        [o.append(l) for o, l in zip(outputs, tf.nest.flatten(last))]
    if reverse:
        outputs = [list(reversed(x)) for x in outputs]
    outputs = [tf.stack(x, 0) for x in outputs]
    # "pack sequence as" returns back to original structure {'stoch': stoch, ...}
